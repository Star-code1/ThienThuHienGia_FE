...
.panels-wrapper
  ...
  .panel-box(
    ...
    .panel-header-title {{ panels.rollCall?.title || 'CHỐT ĐIỂM DANH & VÕ PHÁI' }}
    ...
    .rollcall-footer
      ...
      .footer-stat-row
        span.footer-stat-label Cáo Bận
        span.footer-stat-val.val-red {{ panels.rollCall?.totalBusy || 9 }} đệ tử
  ...
  .panel-box(
    ...
    .panel-header-title.title-red
      template(v-if="!isEditMode")
        span {{ panels.rung1?.title || 'TIÊN PHONG TRÁI' }}
      template(v-else)
    ...
    .leader-card(
      ...
      .leader-avatar-group
        .avatar-circle
          ...
          span.avatar-initial(v-else) {{ panels.rung1?.leaderName?.charAt(0) || 'P' }}
        span.mic-icon 💬
      ...
      .leader-info
        template(v-if="panels.rung1?.leaderName")
          ...
          .leader-subtag {{ panels.rung1.subTag || 'Tiên Phong Trái' }}
        template(v-else)
          .empty-leader-text + Kéo đệ tử thả vào làm Tiên Phong Trái
  ...
  .panel-box(
    ...
    .panel-header-title.title-red
      template(v-if="!isEditMode")
        span {{ panels.rung2?.title || 'TIÊN PHONG PHẢI' }}
      template(v-else)
    ...
    .leader-card(
      ...
      .leader-avatar-group
        .avatar-circle
          ...
          span.avatar-initial(v-else) {{ panels.rung2?.leaderName?.charAt(0) || 'P' }}
        span.mic-icon 💬
      ...
      .leader-info
        template(v-if="panels.rung2?.leaderName")
          ...
          .leader-subtag {{ panels.rung2.subTag || 'Tiên Phong Phải' }}
        template(v-else)
          .empty-leader-text + Kéo đệ tử thả vào làm Tiên Phong Phải
  ...
  .panel-box(
    ...
    .panel-header-title.flex-row-between
      span {{ panels.tactics?.title || 'THIÊN THƯ CHIẾN THUẬT' }}
