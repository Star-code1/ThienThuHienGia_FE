<template>
  <div class="min-h-screen bg-[#060a12] text-[#e2e8f0] flex flex-col font-sans relative overflow-x-auto selection:bg-[#3b82f6]/30">
    <!-- Ambient ambient glow nền gaming -->
    <div
      class="pointer-events-none fixed inset-0 opacity-40 z-0"
      style="background: radial-gradient(60rem 30rem at 15% -10%, rgba(59,130,246,0.12), transparent), radial-gradient(50rem 30rem at 90% 20%, rgba(234,179,8,0.08), transparent);"
    ></div>

    <!-- Header Toolbar -->
    <header class="relative z-20 border-b border-[#172439] bg-[#090f1d]/90 backdrop-blur px-4 py-2.5 flex flex-wrap justify-between items-center gap-3">
      <div class="flex items-center gap-3">
        <!-- Logo & Title -->
        <div class="flex items-center gap-2">
          <span class="text-xl">📜</span>
          <h1 class="font-bold text-lg tracking-wide bg-gradient-to-r from-[#fef08a] via-[#f5c518] to-[#b45309] bg-clip-text text-transparent uppercase font-serif">
            {{ store.title || 'Thiên Thư Hiền Giả' }}
          </h1>
        </div>

        <span class="hidden sm:inline text-[11px] uppercase tracking-widest text-[#64748b] border-l border-[#1e293b] pl-3">
          Sơ Đồ Bang Chiến
        </span>
      </div>
    ...
    <!-- Thanh Legend Phái với Image Assets -->
    <div class="relative z-10 border-b border-[#111c2e] bg-[#070b13] px-4 py-1.5 flex items-center gap-4 overflow-x-auto">
      <span class="text-[10px] uppercase font-bold tracking-widest text-[#475569] shrink-0">Phái</span>
...
import TacticalDivisionBox from './components/TacticalDivisionBox.vue';
import TacticalBanners from './components/TacticalBanners.vue';
